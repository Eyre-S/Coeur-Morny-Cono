...
import cc.sukazyo.cono.morny.system.telegram_api.Standardize.{ChatID, MessageID}
...
import cc.sukazyo.cono.morny.system.telegram_api.message.{Messages, TextMessage}
import cc.sukazyo.cono.morny.system.telegram_api.text.{NativeText, Texts}
import com.pengrad.telegrambot.model.request.ParseMode
import com.pengrad.telegrambot.model.{Chat, ChatFullInfo, Message, MessageEntity}
import com.pengrad.telegrambot.request.{GetChat, SendMessage}
...
class OnCallMsgSend (using coeur: MornyCoeur) extends EventListener {
	...
	case class MessageToSend (
		...
		def toSendMessage (target_override: Long|Null = null): SendMessage =
			...
			sendMessage
		def toMessage (targetOverride: ChatID | Null, replyTo: MessageID): TextMessage =
			val useTarget = if targetOverride == null then targetId else targetOverride
			Messages.create(useTarget.nn)(NativeText(
				Option(message).getOrElse(""),
				Option(parseMode),
				Option(entities).map(_.toSeq).getOrElse(Seq.empty)
			))
	}
	...
	override def onMessage (using event: EventEnv): Unit = {
		...
		if (!(coeur.trusted isTrust message.from))
			ccMsg.sticker(TelegramStickers.ID_403).send
			event.setEventOk
			return;
		...
		if (targetChatResponse isOk) {
			def getChatDescriptionHTML (chat: ChatFullInfo): String =
		...
		val testSendResponse =
			messageToSend.toMessage(update.message.chat.id, update.message.messageId)
				.send
	...
	private def answer404 (using event: EventEnv): Unit =
		Messages.derive(event.update.message)
